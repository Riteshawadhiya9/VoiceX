...
const SignUp = () => {
  ...
  const handleSignUp = async (e) => {
    ...
    // STOP here if any validation failed — do NOT call API
    if (!valid) return

    // --- API CALL --- //
    setLoading(true)
    try {
      let result = await axios.post(`${serverUrl}/api/auth/sign-up`, {
        name, email, password
  ...
  const inputBase = 'w-full h-[52px] pl-11 pr-4 rounded-xl text-white text-[15px] placeholder-white/25 outline-none transition-all duration-200'
  ...
  return (
    ...
    >
      ...
      <form
        ...
        className='relative z-10 w-[90%] max-w-[460px] flex flex-col items-center gap-4 px-8 py-10 rounded-3xl'
        style={{
      ...
      >
        ...
        <div className='flex flex-col items-center gap-3 mb-1'>
          ...
          <div className='text-center'>
            <h1 className='text-white text-[26px] font-bold tracking-tight leading-tight'>Create account</h1>
            <p className='text-white/40 text-sm mt-1'>Register to <span className='text-red-400 font-medium'>Virtual Assistant</span></p>
          </div>
        </div>

        {/* Name */}
        <div className='w-full flex flex-col gap-1'>
          <label className='text-white/50 text-xs font-semibold uppercase tracking-widest pl-1'>Full Name</label>
          <div className='relative'>
            ...
            <input
              ...
              className={inputBase}
              style={{
                background: nameErr ? 'rgba(220,38,38,0.08)' : 'rgba(255,255,255,0.06)',
                border: nameErr ? '1px solid rgba(220,38,38,0.6)' : '1px solid rgba(255,255,255,0.1)',
              }}
            />
          </div>
          {nameErr && (
        ...
        <div className='w-full flex flex-col gap-1'>
          ...
          <div className='relative'>
            ...
            <button type='button'
              ...
              {!showPassword ? <IoMdEye className='w-[22px] h-[22px]' /> : <IoMdEyeOff className='w-[22px] h-[22px]' />}
            </button>
          </div>
          {passwordErr && (
            <p className='text-red-400 text-xs pl-1 flex items-center gap-1'>
        ...
        <p className='text-white/40 text-sm'>
          ...
          <span className='text-red-400 font-semibold cursor-pointer hover:text-red-300 transition-colors'
            onClick={() => navigate("/sign-in")}>Login</span>
        </p>
      </form>
    </div>
  )
